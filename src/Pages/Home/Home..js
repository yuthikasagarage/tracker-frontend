...
const Home = () => {
  ...
  const sort = () => {
    recordParsed?.userRecords?.sort(function (a, b) {
    ...
    <>
      ...
      <div className={classNames.contentWrapper}>
        <div style={{ height: 500, width: "inherit" }}>
          {records && (
            <LineChart records={records} select={setSelectedMeasurement} />
          )}
        </div>
        ...
        >
          {" "}
          <h3>
            Click on the points in the graph to select a measurement to update
            or delete
          </h3>
          ...
          <div style={{ fontWeight: "bold" }}>
            {`Weight :${
              selectedMeasurement ? selectedMeasurement?.data.y : ""
            } `}
          </div>
        </div>
      </div>
